...
Game engine implementations and the `GameEngine` trait.

## Structure

```
games/
├── mod.rs              # Module exports, GameEngine trait
├── registry.rs         # Game registry (ID → factory mapping)
├── common.rs           # Shared utilities (TurnRotation, etc.)
├── error.rs            # Game-specific errors
│
├── lexi_wars/          # Word game implementation
│   ├── mod.rs
│   ├── engine.rs       # LexiWarsEngine
│   ├── message.rs      # Actions and events
│   └── rule.rs         # Word validation rules
│
└── ludo/               # Board game implementation
    ├── mod.rs
    ├── engine.rs       # LudoEngine
    ├── message.rs      # Actions and events
...
Every game must implement the `GameEngine` trait:
...
pub trait GameEngine: Send + Sync {
    ...
    async fn get_game_state(&self) -> serde_json::Value;

    /// Start the game loop (timers, countdowns, etc.)
...
```bash
mkdir -p src/games/my_game
touch src/games/my_game/{mod.rs,engine.rs,message.rs}
```

## Step 3: Define Messages

Create `src/games/my_game/message.rs`:
