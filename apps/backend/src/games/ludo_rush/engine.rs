...
impl LudoRushEngine {
    pub fn new(lobby_id: Uuid, state: AppState) -> Self {
        Self {
            inner: Arc::new(RwLock::new(LudoRushInner::new(lobby_id, state))),
        }
    }
...
async fn run_game_loop(inner: Arc<RwLock<LudoRushInner>>, state: AppState) {
...
pub fn create_ludo_rush(lobby_id: Uuid, state: AppState) -> Box<dyn GameEngine> {
    Box::new(LudoRushEngine::new(lobby_id, state))
}
