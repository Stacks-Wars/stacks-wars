// Ludo Rush Board Logic
//
// Board structure (same as Ludo):
...
// - Only 4 safe squares (each player's entry point)
// - Capturing sends the opponent home AND finishes the attacker's pawn
...
/// Safe squares — only the 4 entry points (one per player)
pub const SAFE_SQUARES: [usize; 4] = [0, 13, 26, 39];
...
impl PlayerBoardState {
    ...
    /// Instantly finish a pawn (used in Ludo Rush when capturing an opponent)
    pub fn finish_pawn(&mut self, pawn_id: usize) {
        self.pawns[pawn_id].position = PawnPosition::Finished;
        self.pawns_finished += 1;
    }
}
...
/// The complete Ludo Rush board state
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct LudoRushBoard {
...
impl LudoRushBoard {
    ...
    /// Check if a position is a safe square (only entry points in Ludo Rush)
...
mod tests {
    ...
    #[test]
    fn test_safe_squares_only_entry_points() {
        // Ludo Rush only has 4 safe squares (the entry points)
        assert!(LudoRushBoard::is_safe_square(0));
        assert!(LudoRushBoard::is_safe_square(13));
        assert!(LudoRushBoard::is_safe_square(26));
        assert!(LudoRushBoard::is_safe_square(39));

        // These are safe in regular Ludo but NOT in Ludo Rush
        assert!(!LudoRushBoard::is_safe_square(8));
        assert!(!LudoRushBoard::is_safe_square(21));
        assert!(!LudoRushBoard::is_safe_square(34));
        assert!(!LudoRushBoard::is_safe_square(47));
    }
    ...
    fn test_finish_pawn() {
        ...
        // Move pawn onto the track first
        player.pawns[0].position = PawnPosition::OnTrack(5);

        // Instantly finish the pawn (Ludo Rush capture reward)
        player.finish_pawn(0);
        assert_eq!(player.pawns[0].position, PawnPosition::Finished);
        assert_eq!(player.pawns_finished, 1);
    }
    ...
    fn test_capture() {
        ...
        let mut board = LudoRushBoard::new(&[user1, user2]);

        // Move player 1's pawn to position 5 (not safe in Ludo Rush)
        ...
        // No capture on safe square (entry point)
        board.get_player_mut(user1).unwrap().pawns[0].position = PawnPosition::OnTrack(0);
        let capture = board.check_capture(user2, 0);
        assert!(capture.is_none());

        // Position 8 is NOT safe in Ludo Rush (unlike regular Ludo)
        board.get_player_mut(user1).unwrap().pawns[0].position = PawnPosition::OnTrack(8);
        let capture = board.check_capture(user2, 8);
        assert!(capture.is_some());
    }
}
