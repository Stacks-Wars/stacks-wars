...
use stacks_wars_core::CoreError;
use thiserror::Error;
...
impl From<AppError> for CoreError {
    fn from(e: AppError) -> Self {
        match e {
            AppError::BadRequest(m) => CoreError::BadRequest(m),
            AppError::NotFound(m) => CoreError::NotFound(m),
            AppError::Serialization(m) => CoreError::Serialization(m),
            AppError::Deserialization(m) => CoreError::Deserialization(m),
            AppError::RedisError(m) | AppError::RedisPoolError(m) => CoreError::Redis(m),
            AppError::DatabaseError(m) => CoreError::Database(m),
...
impl From<CoreError> for AppError {
    fn from(e: CoreError) -> Self {
        match e {
            ...
            CoreError::Serialization(m) => AppError::Serialization(m),
            CoreError::Deserialization(m) => AppError::Deserialization(m),
            CoreError::Redis(m) => AppError::RedisError(m),
            CoreError::Database(m) => AppError::DatabaseError(m),
            CoreError::Internal => AppError::InternalError,
        }
    }
}
