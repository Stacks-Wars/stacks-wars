use stacks_wars_core::{
    CoreError, GameEngine, GameHostRef, GamePlayerState, GameResults, GameRoomBroadcast,
    PlayerRanking, PlayerStateWire, TurnRotation, UserRoomMessage, WarsPointContext,
};
...
impl CheckersInner {
    ...
    fn player_state(&self, user_id: Uuid) -> Option<PlayerStateWire> {
    ...
    fn apply_move(&mut self, mv: CheckersMove) -> Result<(bool, bool), CoreError> {
        let from = mv.from;
        let to = mv.to;
        ...
        if from.row >= BOARD_SIZE
            ...
            || to.col >= BOARD_SIZE
        {
            return Err(CoreError::BadRequest("Move is out of bounds".to_string()));
        }

        let mut piece = self.board[from.row][from.col]
            .ok_or_else(|| CoreError::BadRequest("No piece at source square".to_string()))?;
    ...
    async fn send_game_over(&self, user_id: Uuid, rank: usize, prize: Option<f64>, wars_point: f64) {
        let message = UserRoomMessage::GameOver {
            rank,
            prize,
            wars_point,
        };
        self.host.broadcast_user_room(user_id, &message).await;
    }

    async fn finish_with_standings(&self, standings: Vec<PlayerStateWire>) {
        self.host
            .broadcast_room_game(
                self.lobby_id,
                &GameRoomBroadcast::FinalStanding { standings },
            )
            .await;

        if let Err(e) = self.host.finish_lobby(self.lobby_id).await {
    ...
    async fn end_with_winner(&mut self, winner_id: Uuid) {
        ...
        for (user_id, rank, is_winner) in outcomes {
            ...
            let result = match self
                .host
                .save_player_result_with_winner(self.lobby_id, &ctx, is_winner)
                .await
            {
                Ok(saved) => saved,
                Err(e) => {
    ...
    async fn end_draw(&mut self, reason: String) {
        ...
        for user_id in self.turn_rotation.active_players() {
            ...
            let result = match self
                .host
                .save_player_result_with_winner(self.lobby_id, &ctx, false)
                .await
            {
                Ok(saved) => saved,
                Err(e) => {
    ...
    async fn perform_move(&mut self, user_id: Uuid, from: Position, to: Position) -> Result<bool, CoreError> {
        if self.finished {
            return Err(CoreError::BadRequest("Game already finished".to_string()));
        }
        ...
        let Some(player) = self.player_state(user_id) else {
            return Err(CoreError::NotFound("Player state not found".to_string()));
        };
        ...
        if captured {
            let Some(color) = self.player_colors.get(&user_id).copied() else {
                return Err(CoreError::BadRequest("Player color missing".to_string()));
            };
        ...
        if let Some(current_id) = self.current_player_id() {
            let current_color = self
                ...
                .get(&current_id)
                .copied()
                .ok_or_else(|| CoreError::BadRequest("Current player color missing".to_string()))?;
...
impl CheckersEngine {
    pub fn new(lobby_id: Uuid, host: GameHostRef) -> Self {
        Self {
            inner: Arc::new(RwLock::new(CheckersInner::new(lobby_id, host.clone()))),
            host,
        }
    }
...
impl GameEngine for CheckersEngine {
    ...
    async fn initialize(&mut self, player_ids: Vec<Uuid>) -> Result<Vec<Value>, CoreError> {
        ...
        let lobby_id = { self.inner.read().await.lobby_id };
        let wire_states = self.host.get_player_states_in_lobby(lobby_id).await?;
        ...
        self.host
            .broadcast_room_game(lobby_id, &GameRoomBroadcast::GameStarted)
            .await;
        self.host
            .broadcast_game_message(
                lobby_id,
    ...
    async fn handle_action(&mut self, user_id: Uuid, action: Value) -> Result<Vec<Value>, CoreError> {
        let action: CheckersAction = serde_json::from_value(action)
            .map_err(|e| CoreError::BadRequest(format!("Invalid checkers action: {}", e)))?;
...
async fn run_game_loop(inner: Arc<RwLock<CheckersInner>>, host: GameHostRef) {
    let (notify, lobby_id) = {
...
pub fn create_checkers(lobby_id: Uuid, host: GameHostRef) -> Box<dyn GameEngine> {
    Box::new(CheckersEngine::new(lobby_id, host))
}
