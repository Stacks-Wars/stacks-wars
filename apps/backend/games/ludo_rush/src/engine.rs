...
// - LudoRushEngine struct and implementation
// - GameEngine trait implementation
// - Game loop (turn timeouts)
// - Prize/points calculation
//
// Ludo Rush differences from classic Ludo:
// - Capturing sends the opponent home AND instantly finishes the attacker's pawn
// - Only 4 safe squares (each player's entry point at positions 0, 13, 26, 39)
//
// Dual-dice variant (same as Ludo):
...
use stacks_wars_core::{
    calculate_wars_point, CoreError, GameEngine, GameError, GameHostRef, GamePlayerState,
    GameResults, GameRoomBroadcast, PlayerRanking, PlayerStateWire, TurnRotation, UserRoomMessage,
    WarsPointContext,
};
...
use super::board::{roll_dice, LudoRushBoard, PawnPosition, PAWNS_PER_PLAYER};
use super::message::{LudoRushAction, LudoRushEvent};
...
struct LudoRushInner {
    lobby_id: Uuid,
    players: HashMap<Uuid, GamePlayerState>,
    player_states: HashMap<Uuid, PlayerStateWire>,
...
impl LudoRushInner {
    fn new(lobby_id: Uuid, host: GameHostRef) -> Self {
        Self {
            lobby_id,
            players: HashMap::new(),
            player_states: HashMap::new(),
            turn_rotation: TurnRotation::new(Vec::new()),
            board: LudoRushBoard::new(&[]),
            total_players: 0,
            finished: false,
            results: None,
...
/// Ludo Rush game engine - wraps shared state for thread-safe access
pub struct LudoRushEngine {
    inner: Arc<RwLock<LudoRushInner>>,
}

impl LudoRushEngine {
    pub fn new(lobby_id: Uuid, host: GameHostRef) -> Self {
        Self {
            inner: Arc::new(RwLock::new(LudoRushInner::new(lobby_id, host))),
        }
    }

    /// Get the Arc<RwLock<Inner>> for spawning game loop
    fn get_inner(&self) -> Arc<RwLock<LudoRushInner>> {
        self.inner.clone()
    }
...
impl LudoRushInner {
    ...
    fn get_player_state(&self, user_id: Uuid) -> Option<PlayerStateWire> {
        self.player_states.get(&user_id).cloned()
    }

    /// Get the current player's PlayerState
    fn get_current_player_state(&self) -> Option<PlayerStateWire> {
        self.turn_rotation
            .current_player()
            .and_then(|id| self.player_states.get(&id).cloned())
    }

    /// Calculate prize for a given rank — winner takes all in Ludo Rush
    ...
    async fn start_turn(&mut self) {
        ...
        let turn_event = LudoRushEvent::Turn {
            player: current_player_state,
            timeout_secs: ROLL_TIMEOUT_SECS,
        };
        self.host.broadcast_game_message(
            self.lobby_id,
    ...
    async fn handle_roll_dice(&mut self, user_id: Uuid) -> Result<bool, GameError> {
        ...
        if self.turn_phase != TurnPhase::WaitingForRoll {
            let event = LudoRushEvent::Invalid {
                reason: "You've already rolled the dice".to_string(),
            };
            self.host.broadcast_game_message_to_user(
                self.lobby_id,
                user_id,
        ...
        // Broadcast dice roll
        let roll_event = LudoRushEvent::DiceRolled {
        ...
        if playable.is_empty() {
            let event = LudoRushEvent::NoValidMoves {
                player: player_state,
            };
            self.host.broadcast_game_message(
                self.lobby_id,
    ...
    async fn handle_select_dice_value(&mut self, user_id: Uuid, dice_value: u8) -> Result<bool, GameError> {
        ...
        let event = LudoRushEvent::MovablePawns {
            dice_value,
            pawns: movable,
        };
        self.host.broadcast_game_message_to_user(
    ...
    ///
    /// Ludo Rush capture mechanic:
    /// When capturing an opponent's pawn, the opponent's pawn is sent home
    /// AND the attacker's pawn is instantly finished (moved to Finished state).
    async fn handle_move_pawn(&mut self, user_id: Uuid, pawn_id: usize) -> Result<bool, GameError> {
        ...
        let dice_value = match self.selected_dice_value {
            ...
            None => {
                ...
                self.host
                    .broadcast_game_message_to_user(
                        self.lobby_id,
                        user_id,
                        serde_json::to_value(&event).unwrap_or_default(),
                    )
        ...
        let move_event = LudoRushEvent::PawnMoved {
            player: player_state.clone(),
            pawn_id,
            from: from_position,
            to: to_position,
        ...
        // Ludo Rush: capturing sends opponent home AND finishes the attacker's pawn
        if let PawnPosition::OnTrack(pos) = to_position {
            if let Some((victim_id, victim_pawn_id)) = self.board.check_capture(user_id, pos) {
                ...
                // Finish the attacker's pawn (Ludo Rush reward)
                self.board
                    .get_player_mut(user_id)
                    .unwrap()
                    .finish_pawn(pawn_id);
                ...
                let capture_event = LudoRushEvent::PawnCaptured {
                    attacker: player_state.clone(),
                    victim: victim_state,
                    victim_pawn_id,
                    attacker_pawn_id: pawn_id,
                };
                self.host.broadcast_game_message(
                    self.lobby_id,
                ...
                // Check if the attacker won by finishing this pawn
                if self.board.get_player(user_id).unwrap().has_won() {
                    self.consume_dice_value(dice_value);
                    self.turn_phase = TurnPhase::Complete;

                    // Broadcast board update before ending
                    let board_event = LudoRushEvent::BoardUpdate {
                ...
                // Broadcast PawnFinished for the attacker's pawn (since it was instantly finished)
                let pawns_remaining = PAWNS_PER_PLAYER
                    - self.board.get_player(user_id).unwrap().pawns_finished;

                let finish_event = LudoRushEvent::PawnFinished {
                    player: player_state.clone(),
                    pawn_id,
                    pawns_remaining,
                };
        ...
        // Check if pawn finished (via normal movement, not capture)
        ...
        let used_event = LudoRushEvent::DiceValueUsed {
            dice_value,
        ...
        if both_used {
            ...
            if let Some((d1, d2)) = self.dice {
                if d1 == 6 && d2 == 6 {
                    let bonus_event = LudoRushEvent::BonusTurn {
                        player: player_state,
                    };
                    self.host
                        .broadcast_game_message(
    ...
    async fn eliminate_player(&mut self, player_id: Uuid, reason: &str) {
        ...
        let wars_point = match self.host.save_player_result(self.lobby_id, &ctx).await {
        ...
        if let Some(player) = self.player_states.get(&player_id).cloned() {
            let event = LudoRushEvent::PlayerQuit {
                player,
    ...
    async fn end_game_last_standing(&mut self) {
        ...
        let mut final_standings: Vec<PlayerStateWire> = Vec::new();
        ...
        for &winner_id in &active_players {
            ...
            let wars_point = match host.save_player_result(lobby_id, &ctx).await {
                Ok(result) => result.wars_point,
                Err(e) => {
                    tracing::error!("Failed to save player result: {}", e);
                    calculate_wars_point(&ctx)
        ...
        host.broadcast_room_game(
            lobby_id,
            &GameRoomBroadcast::FinalStanding {
                standings: final_standings,
            },
        )
        .await;

        if let Err(e) = host.finish_lobby(lobby_id).await {
...
impl GameEngine for LudoRushEngine {
    ...
    async fn initialize(&mut self, player_ids: Vec<Uuid>) -> Result<Vec<Value>, CoreError> {
        tracing::info!("Initializing Ludo Rush with {} players", player_ids.len());

        if player_ids.len() < 2 {
            let error_event = GameRoomBroadcast::GameStartFailed {
                reason: "Ludo Rush requires at least 2 players".to_string(),
            };
        ...
        if player_ids.len() > 4 {
            let error_event = GameRoomBroadcast::GameStartFailed {
                reason: "Ludo Rush supports at most 4 players".to_string(),
            };
            return Ok(vec![serde_json::to_value(&error_event).unwrap_or_default()]);
        }
        ...
        inner.board = LudoRushBoard::new(&player_ids);

        match inner
            .host
            .get_player_states_in_lobby(inner.lobby_id)
            .await
        {
            Ok(states) => {
                for ps in states {
            ...
            Err(e) => tracing::error!("Failed to load player states: {}", e),
        }

        inner
            .host
            .broadcast_room_game(inner.lobby_id, &GameRoomBroadcast::GameStarted)
            .await;
    ...
    async fn handle_action(
        ...
        if inner.finished {
            return Err(CoreError::BadRequest("Game is already finished".into()));
        }

        let action: LudoRushAction = serde_json::from_value(action)
            .map_err(|e| CoreError::BadRequest(format!("Invalid action: {}", e)))?;

        tracing::debug!("Ludo Rush action from {}: {:?}", user_id, action);

        let should_advance = match action {
            LudoRushAction::RollDice => inner.handle_roll_dice(user_id).await,
            LudoRushAction::SelectDiceValue { dice_value } => inner.handle_select_dice_value(user_id, dice_value).await,
            LudoRushAction::MovePawn { pawn_id } => inner.handle_move_pawn(user_id, pawn_id).await,
        };
        ...
        match should_advance {
            ...
            Err(e) => {
                tracing::warn!("Ludo Rush action error: {}", e);
            }
        }
    ...
    async fn get_game_state(&self, _user_id: Option<Uuid>) -> Result<Value, CoreError> {
        let inner = self.inner.read().await;

        let current_player = inner.get_current_player_state();
        let turn = current_player.map(|player| LudoRushEvent::Turn {
            player,
            timeout_secs: ROLL_TIMEOUT_SECS,
        });
    ...
    async fn handle_player_quit(&mut self, user_id: Uuid) -> Result<Vec<Value>, CoreError> {
        let mut inner = self.inner.write().await;
    ...
    fn start_loop(&mut self, host: GameHostRef) {
        let inner = self.get_inner();
        tokio::spawn(run_game_loop(inner, host));
    }
}
...
/// Run the game loop for Ludo Rush
/// This handles turn timeouts and countdown broadcasts
async fn run_game_loop(inner: Arc<RwLock<LudoRushInner>>, host: GameHostRef) {
    ...
    loop {
        ...
        if is_finished || has_winner {
            tracing::info!("Ludo Rush game loop ending for lobby {}", lobby_id);
            break;
        }
        ...
        run_turn(&inner, &host, &turn_advance_notify, lobby_id).await;
...
async fn run_phase_countdown(
    ...
    while time_remaining > 0 {
        // Broadcast countdown
        let countdown_event = LudoRushEvent::Countdown {
            time: time_remaining,
        };
        host.broadcast_game_message(
            lobby_id,
    ...
    let countdown_event = LudoRushEvent::Countdown { time: 0 };
    host.broadcast_game_message(
        lobby_id,
        serde_json::to_value(&countdown_event).unwrap_or_default(),
    )
...
/// Factory function to create new Ludo Rush game instances
pub fn create_ludo_rush(lobby_id: Uuid, host: GameHostRef) -> Box<dyn GameEngine> {
    Box::new(LudoRushEngine::new(lobby_id, host))
}
