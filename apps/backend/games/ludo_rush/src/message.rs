...
pub enum LudoRushEvent {
    ...
    #[serde(rename_all = "camelCase")]
    PawnCaptured {
        attacker: PlayerStateWire,
        victim: PlayerStateWire,
    ...
    NoValidMoves { player: PlayerStateWire },

    /// Bonus turn awarded (both dice rolled 6) - broadcast to room
    BonusTurn { player: PlayerStateWire },

    /// Player quit the game - broadcast to room
    PlayerQuit { player: PlayerStateWire, reason: String },
