...
impl LudoEngine {
    pub fn new(lobby_id: Uuid, host: GameHostRef) -> Self {
        Self {
            inner: Arc::new(RwLock::new(LudoInner::new(lobby_id, host))),
        }
    }
...
async fn run_game_loop(inner: Arc<RwLock<LudoInner>>, host: GameHostRef) {
...
pub fn create_ludo(lobby_id: Uuid, host: GameHostRef) -> Box<dyn GameEngine> {
    Box::new(LudoEngine::new(lobby_id, host))
}
