...
use stacks_wars_core::{
    ...
    GameResults, GameRoomBroadcast, PlayerStateWire, TurnRotation, UserRoomMessage,
    WarsPointContext,
};
...
impl LexiWarsEngine {
    pub fn new(lobby_id: Uuid, host: GameHostRef) -> Self {
        Self {
            inner: Arc::new(RwLock::new(LexiWarsInner::new(lobby_id, host))),
        }
    }
...
impl LexiWarsInner {
    ...
    /// Get a player's wire state by ID
    ...
    async fn end_game(&mut self) {
        ...
        for ranking in &results.rankings {
            ...
            let wars_point = if is_active {
                ...
                match host.save_player_result(lobby_id, &ctx).await {
                    Ok(result) => result.wars_point,
                    Err(e) => {
                        tracing::error!("Failed to save player result: {}", e);
    ...
    async fn start_turn(&mut self) {
        ...
        self.host.broadcast_game_message_to_room_except(
            self.lobby_id,
            current_player_id,
...
impl GameEngine for LexiWarsEngine {
    ...
    async fn initialize(&mut self, player_ids: Vec<Uuid>) -> Result<Vec<Value>, CoreError> {
        ...
        if player_ids.len() < 2 {
            let event = GameRoomBroadcast::GameStartFailed {
    ...
    async fn handle_action(
        ...
        let action: LexiWarsAction = serde_json::from_value(action)
            .map_err(|e| CoreError::BadRequest(format!("Invalid LexiWars action: {}", e)))?;
...
async fn run_game_loop(inner: Arc<RwLock<LexiWarsInner>>, host: GameHostRef) {
...
pub fn create_lexi_wars(lobby_id: Uuid, host: GameHostRef) -> Box<dyn GameEngine> {
    Box::new(LexiWarsEngine::new(lobby_id, host))
}
