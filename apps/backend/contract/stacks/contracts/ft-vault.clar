;; title: stacks wars ft vault
;; author: flames.stx
;; version: v1
;; summary: A fungible token vault contract for Stacks Wars.
;; description: A fungible token vault contract for Stacks Wars, allowing players to join
...
(define-public (join)
	(let
		...
		(try! (contract-call? .STACKS-WARS-TOKEN transfer ENTRY-FEE sender (as-contract tx-sender) none))
...
(define-public (leave (signature (buff 65)))
	(let
		...
		(try! (as-contract (contract-call? .STACKS-WARS-TOKEN transfer ENTRY-FEE tx-sender sender none)))
...
(define-public (kick (player principal) (signature (buff 65)))
	(let
		...
		;; Transfer entry fee from contract back to player
		(try! (as-contract (contract-call? .STACKS-WARS-TOKEN transfer ENTRY-FEE tx-sender player none)))
